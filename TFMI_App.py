...
class TFdata:
    ...
    def getfilename(self, directory, file_tag, num=0):
        fname = r"{}\{}__{}.dat".format(directory, file_tag, num)
        if os.path.isfile(fname):
            return fname
        return self.getfilename(directory, file_tag, num+1)
        
    def save_fits(self):
        fname = self.getfilename(self.save_directory, "{}_{}".format(self.TF_name, self.today))
        if type(self.header) == None:
            np.savetxt(fname, self.fits, delimiter='\t')
        else:
            np.savetxt(fname, self.fits, header=self.header, delimiter='\t')
            
    def save_sweep(self, fname):
        fname = self.getfilename(self.save_directory, "\sweeps_{}\{}_{}".format(self.today, self.TF_name, self.today))
        if type(self.sweep_header) == None:
            np.savetxt(fname, self.sweep, delimiter='\t')
        else:
            np.savetxt(fname, self.sweep, header=self.sweep_header, delimiter='\t')
...
class tkApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.data = TFdata(TF_name="test", save_directory=r"data", drive = 1)
        self.params = {
        ...
        self.fit_check.pack(in_=self.bottom1, side=tk.LEFT, padx=5, pady=5)
        ...
        self.track_check.pack(in_=self.bottom1, side=tk.LEFT, padx=5, pady=5)
        ...
        self.showGraph_Options.pack(in_=self.bottom1, side=tk.LEFT, padx=5, pady=5)
    ...
    def sweep(self):
        ...
        for idx, f in enumerate(frequencies):
            ...
            self.TFdata.append([time.time(), f, Vx, Vy])
            if self.showGraph.get() == "Frequency Sweep":
                self.graph_sweep()
    ...
    def start(self):
        ...
        while True:
            try:
                ...
                if np.array(self.TFdata.fits).ndim > 1:
